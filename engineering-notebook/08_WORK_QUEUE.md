...
| SM-W003 | Owner architecture review: accept, modify, reject, or defer proposed decisions SM-P001–SM-P020. | Owner | BLOCKED | SM-W002 refinement | Updated decision register / architecture direction. |
...
| SM-W008 | Produce the first executable headless vertical-slice plan using the evolving **Explainer Swarm** proof of concept. The slice must include the baseline Memory Manager architecture even if the initial role set is reduced. | Daisy | BLOCKED | SM-W004, SM-W006, SM-W007, SM-W016, SM-W019 | Bounded API/CLI implementation work orders. |
...
| SM-W010 | Implement ChatGPT/Claude runtime adapters, Boot Manifest compilation, commissioning suite, and the runtime hooks required for Memory Manager durable-memory exchange and recall challenges. | Worker + Daisy review | BLOCKED | SM-W005, SM-W009, SM-W016 | End-to-end commissioning evidence. |
| SM-W011 | Prove create→commission→reconcile→archive vertical slice with the Explainer Swarm without destructive deletion, including baseline Memory Manager recovery/reconciliation evidence. | Worker + Daisy review | BLOCKED | SM-W010, SM-W024 | Verified lifecycle evidence. |
...
| SM-W016 | **Define the baseline Memory Manager architecture required in every swarm:** canonical memory schema/classes, Memory Store provider contract, durable-memory intake/output mediation, recurrence/reminder semantics, recall/reconciliation protocol, drift classifications, transfer-prompt generation, user governance/edit/delete rules, and storage-backend evaluation for local directory vs Git vs Context Keep/hybrid. | Daisy / Worker | READY_FOR_DESIGN | memory protocol maturity | Versioned baseline Memory Manager design and proof fixtures; `13_MEMORY_MANAGER_BASELINE.md`. |
...
| SM-W018 | Implement Memory Manager dashboard surfaces and future Token Monitor dashboard modules, with swarm summary and per-agent drill-down; final one-page-vs-separate-page layout decided from usability evidence. | Worker + Daisy review | FUTURE | SM-W012, SM-W015, SM-W024, dashboard module framework | Working modules, telemetry/reconciliation tests, user-management flows. |
| SM-W019 | **Create and continuously evolve the Explainer Swarm blueprint as the principal product proof of concept.** Begin with the Presenter/SME/Researcher/Curriculum/Writer/Reviewer model and refine roles, interactions, baseline Memory Manager, optional modules, and commissioning as Swarm Manager matures. | Daisy + Owner review | READY | none | Versioned Explainer blueprint history and POC acceptance notes. |
...
| SM-W024 | **Implement and commission the mandatory Memory Manager baseline for every swarm.** Provide the configured Memory Store backend, durable-memory mediation API, reminders/reinforcement, recall/reconciliation runs, drift log, user-governed memory operations, and continuously maintained per-agent transfer/recovery prompts. | Worker + Daisy review | BLOCKED | SM-W009, SM-W010, SM-W016 | Memory Manager service/module, backend adapter(s), recovery prompts, reconciliation evidence, commissioning checks. |
...
5. **SM-W016** — design the mandatory Memory Manager baseline and storage-provider abstraction.
6. **SM-W019** — begin the Explainer Swarm blueprint and keep evolving it as the standing proof of concept.
7. **SM-W020** — evaluate reusable OSS workflow/graph editors for the Swarm Creation Tool.
8. **SM-W022** — design per-swarm UI deployment and 6000–7000 port allocation.

Normal provisioning is **library-first**. Swarm authoring is a separate tool and workstream.

**Memory Manager is now mandatory baseline swarm infrastructure and must be present in the first lifecycle proof.** Token Monitor and the optional Token & Memory Steward remain future capabilities layered on top of the stable lifecycle, runtime-observability, and Memory Manager foundations.
